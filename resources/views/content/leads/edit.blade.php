        ...
        <div class="col-xl">
            <div class="card mb-5">
                ...
                <div class="card-body mt-4">
                    ...
                    <form id="lead-form" action="{{ route('leads.update', $lead->id) }}" method="POST">
                        @csrf
                        <div class="row">
                            <!-- Left Column -->
                            <div class="col-md-6">
                                                                    ...
                                                                    <select name="marketing_company" id="marketing_company" class="form-control select2">
                                                                        ...
                                                                        @if($companies->count() >= 0)
                                                                            @foreach ($companies as $company)
                                                                                <option value="{{ $company->id }}" {{ (isset($lead->extraInfo->company_id) && $lead->extraInfo->company_id == $company->id) ? 'selected' : '' }}>{{ $company->name }}</option>
                                                                            @endforeach
                                                                        @endif
                                                                    </select>
                                                                    ...
                                                                    <select name="marketing_campaign" id="marketing_campaign" class="form-control select2">
                                                                        ...
                                                                        @if($campaigns->count() >= 0)
                                                                            @foreach ($campaigns as $campaign)
                                                                                <option value="{{ $campaign->id }}" {{ (isset($lead->extraInfo->campaign_id) && $lead->extraInfo->campaign_id == $campaign->id) ? 'selected' : '' }}>{{ $campaign->name }}</option>
                                                                            @endforeach
                                                                        @endif
                                                                    </select>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("click", ".save-lead", function(e) {
                ...
                if (status === 'opportunity' || status === 'lost') {
                    ...
                    $('#noteStatusGroup').html(`
                        <div class="mb-2">
                            <label><input type="radio" name="note_status" value="${status}" checked> ${status === 'opportunity' ? 'Opportunity' : 'Lost'} Notes</label>
                        </div>
                    `);
